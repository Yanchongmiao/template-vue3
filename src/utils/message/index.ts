...
import { MsgModalOptions, MsgOptions } from "../../type";
/**
...
export const createSuccessMsg = (opiions: MsgOptions) => {
  notification.open({
    message: createVNode('b', { style: 'color:#303133;font-weight: 700;font-size: 16px' }, `${opiions.title}`),
    description: opiions.content,
    icon: h('span', { style: 'color: #67c23a', class: `yc-iconfont ${opiions.iconClass || 'yc-icon-icon_smile'} ` }),
...
export const createErrorMsg = (opiions: MsgOptions) => {
  notification.open({
    message: createVNode('b', { style: 'color:#303133;font-weight: 700;font-size: 16px' }, `${opiions.title}`),
    description: opiions.content,
    icon: h('span', { style: 'color: red', class: `yc-iconfont ${opiions.iconClass || 'yc-icon-ku'} ` }),
    duration: opiions.time || 3,
    key: opiions.key || String(Date.now() + (Math.ceil(Math.random() * 1000)))
  });
}
export const createInfoMsg = (opiions: MsgOptions) => {
  notification.open({
    message: createVNode('b', { style: 'color:#303133;font-weight: 700;font-size: 16px' }, `${opiions.title}`),
    description: opiions.content,
    icon: h('span', { style: 'color: #909399', class: `yc-iconfont ${opiions.iconClass || 'yc-icon-tishi1'} ` }),
    duration: opiions.time || 3,
    key: opiions.key || String(Date.now() + (Math.ceil(Math.random() * 1000)))
  });
}
export const createWarningMsg = (opiions: MsgOptions) => {
  notification.open({
    message: createVNode('b', { style: 'color:#303133;font-weight: 700;font-size: 16px' }, `${opiions.title}`),
    description: opiions.content,
    icon: h('span', { style: 'color:#e6a23c', class: `yc-iconfont ${opiions.iconClass || 'yc-icon-tixing'} ` }),
    duration: opiions.time || 3,
    key: opiions.key || String(Date.now() + (Math.ceil(Math.random() * 1000)))
  });
}
...
export const createSuccessModal = (opiions: MsgModalOptions) => {
  Modal.success({
    title: opiions.title,
    icon: h('span', {
      style: `float: left;margin:-6px  16px 0 0;font-size: 22px;color:#67c23a;`, class: `yc-iconfont ${opiions.iconClass || 'yc-icon-icon_smile'} `
    }),
...
export const createErrorModal = (opiions: MsgModalOptions) => {
  Modal.error({
    ...
    icon: h('span', {
      style: `float: left;margin:-6px  16px 0 0;font-size: 22px;color:red;`, class: `yc-iconfont ${opiions.iconClass || 'yc-icon-ku'} `
    }),
    content: createVNode('div', { style: 'margin-left:38px' }, opiions.content,),
    okText: opiions.okText || '确定',
    mask: opiions.mask || true,
...
export const createWarningModal = (opiions: MsgModalOptions) => {
  Modal.warning({
    title: createVNode('b', { style: 'color:#303133;font-weight: 700;font-size: 16px' }, `${opiions.title}`),
    icon: h('span', {
      style: `float: left;margin:-6px  16px 0 0;font-size: 24px;color:#e6a23c;`, class: `yc-iconfont ${opiions.iconClass || 'yc-icon-tixing'} `
    }),
    content: createVNode('div', { style: 'margin-left:38px' }, opiions.content,),
    okText: opiions.okText || '确定',
    mask: opiions.mask || true,
